...
export default function EditQRCodePage() {
    ...
    try {
      ...
      if (data.success) {
        ...
        setFormData({
          ...
          nameEn: qr.nameEn || "",
          type: qr.type || "branch",
          branchId: qr.branchId?._id || qr.branchId || "",
          tableNumber: qr.tableNumber || "",
          categoryId: qr.categoryId?._id || qr.categoryId || "",
        ...
        if (qr.customization) {
          setCustomization({
            ...
            borderStyle: qr.customization.borderStyle || "square",
            size: qr.customization.size || 300,
            errorCorrectionLevel: qr.customization.errorCorrectionLevel || "M",
            logoUrl: qr.customization.logoUrl || "",
          });
        }
      } else {
        setMessage({ type: "error", text: "فشل تحميل بيانات رمز QR" });
      }
    } catch (error) {
  ...
  const getPreviewUrl = () => {
    ...
    let url = `${baseUrl}/menu?qr=${formData.token || 'PREVIEW'}`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        setMessage({ type: "success", text: "تم تحديث رمز QR بنجاح" });
        setTimeout(() => {
          router.push("/admin/qrcodes");
        }, 1500);
      } else {
        setMessage({ type: "error", text: data.error || "فشل تحديث رمز QR" });
      }
    } catch (error) {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 md:mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-foreground">تعديل رمز QR</h1>
              <p className="text-sm md:text-base text-muted-foreground mt-1">قم بتعديل معلومات وتخصيصات رمز QR</p>
            </div>
          </div>

          <button
            onClick={handleDelete}
            className="flex items-center justify-center gap-2 px-4 py-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-lg font-medium transition-all"
          >
            ...
            <div className="bg-card rounded-xl p-4 md:p-6 border border-border">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <div
                    key={type.value}
                    className={`flex flex-col items-center gap-2 p-3 md:p-4 rounded-lg border-2 ${
          ...
          <div className="lg:sticky lg:top-6 h-fit">
            <div className="bg-card rounded-xl p-4 md:p-6 border border-border">
              ...
              <div className="space-y-3">
                ...
                <div className="p-3 bg-background/50 rounded-lg">
                  <p className="text-xs text-muted-foreground mb-1">الرمز:</p>
                  <p className="text-xs text-foreground font-mono">{formData.token}</p>
                </div>
              </div>
            </div>
          </div>
