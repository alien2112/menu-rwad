...
export default function NewBranchPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        setMessage({ type: "success", text: "تم إنشاء الفرع بنجاح" });
        setTimeout(() => {
          router.push("/admin/branches");
        }, 1500);
      } else {
        setMessage({ type: "error", text: data.error || "فشل إنشاء الفرع" });
      }
    } catch (error) {
      console.error("Error creating branch:", error);
      setMessage({ type: "error", text: "حدث خطأ أثناء إنشاء الفرع" });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-background p-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">إضافة فرع جديد</h1>
            <p className="text-muted-foreground mt-1">قم بإضافة فرع جديد للمطعم</p>
          </div>
        </div>
