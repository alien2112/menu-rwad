...
import { ShoppingCart, Star, Clock, Flame, Settings } from "lucide-react";
...
export const MenuItemCard = ({
  ...
  const hasCustomization = (sizeOptions && sizeOptions.length > 0) || 
                         (addonOptions && addonOptions.length > 0) || 
                         (dietaryModifications && dietaryModifications.length > 0);
  ...
  const handleCustomizedAddToCart = (customization: any, finalPrice: number) => {
    dispatch({
      type: 'ADD_ITEM',
      payload: {
        id: `${nameAr}-${nameEn}`,
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-shrink-0 relative">
              ...
              {/* Customization Badge */}
              {hasCustomization && (
                <div className="absolute -bottom-1 -left-1 bg-blue-500 text-white px-2 py-1 rounded-full text-xs font-bold shadow-lg z-10">
                  قابل للتخصيص
                </div>
              )}
            </div>
              ...
              <div className="flex items-center gap-2">
                ...
                {hasCustomization && (
                  <button
                    ...
                    className="bg-blue-500/20 hover:bg-blue-500/30 text-blue-300 p-2.5 rounded-full transition-all duration-300 border border-blue-500/30 hover:border-blue-500/50 hover:scale-105"
                    title="تخصيص الطلب"
                  >
                ...
                >
                  ...
                  <span className="relative z-10">
                    {hasCustomization ? 'تخصيص' : 'أضف للسلة'}
                  </span>
                </button>
              </div>
            )}
      ...
      {showCustomizationModal && (
        <MenuItemCustomizationModal
          item={{
            ...
            reviewCount,
            categoryId: category,
            status: status === 'out' ? 'out_of_stock' : status === 'inactive' ? 'inactive' : 'active',
            sizeOptions,
            addonOptions,
            dietaryModifications,
          }}
